...
const CARD_EDITOR_TEXT = {
  en: {
    ...
    "Statusfarben":"Status colors", "Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.":"Colors for overdue, due-soon, and not-yet-due tasks.", "Überfällig":"Overdue", "Bald fällig":"Due soon", "Nicht fällig":"Not due",
...
CARD_EDITOR_TEXT.fr = {
  ...
  "Statusfarben":"Couleurs d’état", "Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.":"Couleurs des tâches en retard, bientôt dues et pas encore dues.", "Überfällig":"En retard", "Bald fällig":"Bientôt due", "Nicht fällig":"Pas encore due",
...
CARD_EDITOR_TEXT.es = {
  ...
  "Statusfarben":"Colores de estado", "Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.":"Colores para tareas atrasadas, próximas a vencer y aún no vencidas.", "Überfällig":"Atrasada", "Bald fällig":"Próxima a vencer", "Nicht fällig":"Aún no vencida",
...
CARD_EDITOR_TEXT.nl = {
  ...
  "Statusfarben":"Statuskleuren", "Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.":"Kleuren voor achterstallige, binnenkort vervallende en nog niet vervallende taken.", "Überfällig":"Achterstallig", "Bald fällig":"Binnenkort vervallen", "Nicht fällig":"Nog niet vervallen",
...
CARD_EDITOR_TEXT.ja = {
  ...
  "Statusfarben":"ステータスカラー", "Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.":"期限切れ、期限間近、期限前のタスクに使用する色です。", "Überfällig":"期限切れ", "Bald fällig":"期限間近", "Nicht fällig":"期限前",
...
function safeStatusColor(value, fallback) {
  const color = String(value || "").trim();
...
class RemeysTaskboardCard extends HTMLElement {
  ...
  _validateEntityConnector(form, connector) {
    ...
    if (!connector.entity_id) {
      entityInput?.setCustomValidity(this._text("entityRequired"));
      entityInput?.reportValidity();
      return false;
    }
    if ((connector.type || "number") === "number") {
      if (!Number.isFinite(Number(connector.due_rule?.value))) {
        dueValueInput?.setCustomValidity(this._text("numericRequired"));
        dueValueInput?.reportValidity();
        return false;
      }
      if (!Number.isFinite(Number(connector.done_rule?.value))) {
        doneValueInput?.setCustomValidity(this._text("numericRequired"));
        doneValueInput?.reportValidity();
        return false;
      }
    }
    if (entityRulesOverlap(connector)) {
      doneValueInput?.setCustomValidity(this._text("rulesOverlap"));
      doneValueInput?.reportValidity();
      return false;
    }
    return true;
  ...
  _taskMarkup(task, column = false, areaOverride = null, showOverdueAge = false) {
    ...
    const entityNotice = entityStatus === "missing"
      ? this._text("entityMissing")
      : entityStatus === "bad" ? this._text("invalidEntityState") : "";
    ...
    const dueMarkup = this._config.list_sort === "last_done"
      ? `<div class="due last-done">${escapeHtml(this._lastDoneLabel(task))}</div>`
      : `<div class="due ${status}">${escapeHtml(this._dueLabel(task))}</div>`;
    const buttonsMarkup = `<span class="task-buttons">${hasNotes ? `<button class="notes-task task-action" draggable="false" title="${escapeHtml(this._text("notes"))}"><ha-icon icon="mdi:paperclip"></ha-icon></button>` : ""}<button class="edit-task task-action" draggable="false" title="${escapeHtml(this._text("edit"))}"><ha-icon icon="mdi:pencil-outline"></ha-icon></button></span>`;
    const details = column
      ...
      : `<div class="details"><div class="name">${escapeHtml(task.Task || "Unbenannte Aufgabe")}</div><div class="list-middle"><div class="task-meta">${showArea ? `<span><ha-icon icon="mdi:map-marker-outline"></ha-icon>${escapeHtml(area)}</span>` : ""}${assignees.map((assignee) => `<span><ha-icon icon="mdi:account-outline"></ha-icon>${escapeHtml(assignee)}</span>`).join("")}<span><ha-icon icon="mdi:repeat"></ha-icon>${escapeHtml(String(task.Rhythmen ?? 0))} ${escapeHtml(rhythmUnit)}</span>${entityStatus === "done" ? `<span class="entity-function done"><ha-icon icon="mdi:function-variant"></ha-icon>${escapeHtml(this._text("doneFunction"))}</span>` : entityStatus === "due" ? `<span class="entity-function due-function"><ha-icon icon="mdi:function-variant"></ha-icon>${escapeHtml(this._text("dueFunction"))}</span>` : entityNotice ? `<span class="entity-function problem"><ha-icon icon="mdi:alert-circle-outline"></ha-icon>${escapeHtml(entityNotice)}</span>` : ""}</div><div class="list-actions">${dueMarkup}${buttonsMarkup}</div></div>${progressBar}</div>`;
  ...
  _taskFormMarkup(task = null, editing = false) {
    ...
    const t = (key, ...args) => escapeHtml(this._text(key, ...args));
    ...
    return `<form class="task-editor-form" data-task-id="${escapeHtml(String(row.uid || row.__uid || row._uuid || ""))}" data-week-mask="${weekMask}" data-month-mask="${monthMask}">
      ...
      <details class="entity-connector" ${connector.enabled ? "open" : ""}><summary><span>${t("entityConnector")}</span><label><input type="checkbox" name="entity_enabled" ${connector.enabled ? "checked" : ""}> ${t("enabled")}</label></summary><div class="entity-fields">
        ...
        <label class="field">${t("dataType")}<select name="entity_type"><option value="number" ${(connector.type || "number") === "number" ? "selected" : ""}>${t("number")}</option><option value="text" ${connector.type === "text" ? "selected" : ""}>${t("text")}</option></select></label>
        <div class="entity-rule"><strong>${t("dueWhen")}</strong><select name="entity_due_op">${operationOptions(connector.due_rule?.op, "<=")}</select><input name="entity_due_value" value="${escapeHtml(connector.due_rule?.value ?? "")}" placeholder="${t("valueExample", 25)} dry"></div>
        <div class="entity-rule"><strong>${t("doneWhen")}</strong><select name="entity_done_op">${operationOptions(connector.done_rule?.op, ">")}</select><input name="entity_done_value" value="${escapeHtml(connector.done_rule?.value ?? "")}" placeholder="${t("valueExample", 35)} wet"></div>
      </div></details>
  ...
  _progressMetrics(task) {
    ...
    const color = turns > 1 ? "var(--rtb-overdue-color)" : turns >= .75 ? "var(--rtb-due-soon-color)" : "var(--rtb-not-due-color)";
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      .task:hover{border-color:color-mix(in srgb,var(--rtb-accent) 45%,var(--rtb-task-border));background-color:color-mix(in srgb,var(--rtb-accent) 7%,transparent)}.task.disabled{opacity:.55;cursor:wait;pointer-events:none}.status-accent{position:absolute;inset:0 auto 0 0;width:4px}.status-accent.status-overdue{background:var(--rtb-overdue-color)}.status-accent.status-soon{background:var(--rtb-due-soon-color)}.status-accent.status-ok{background:var(--rtb-not-due-color)}.task-icon{display:grid;place-items:center;width:${compact ? "34px" : "42px"};height:${compact ? "34px" : "42px"};border:1px solid color-mix(in srgb,var(--rtb-task-border) 75%,transparent);border-radius:50%;background:var(--rtb-icon-bg);color:var(--state-icon-color);flex:none;box-sizing:border-box}.task-icon-inner{display:grid;place-items:center;width:100%;height:100%;border-radius:50%}.task-icon.progress-ring{padding:3px;border:0;background:conic-gradient(var(--progress-color) var(--progress-pct),color-mix(in srgb,var(--divider-color) 55%,transparent) 0)}.progress-ring .task-icon-inner{background:var(--ha-card-background,var(--card-background-color));box-shadow:inset 0 0 0 1px color-mix(in srgb,var(--divider-color) 65%,transparent)}.task-icon ha-icon{--mdc-icon-size:${compact ? "19px" : "23px"}}iconify-icon{font-size:${compact ? "19px" : "23px"}}.external-icon{display:block;width:1.35em;height:1.35em;object-fit:contain}.external-icon.load-error{visibility:hidden}.text-icon{font-size:1.15rem;line-height:1}.task-buttons{display:flex;align-items:center;flex:none}.task-action{border:0;background:none;color:var(--secondary-text-color);cursor:pointer;padding:3px;border-radius:50%;line-height:0}.task-action:hover{color:var(--rtb-accent);background:color-mix(in srgb,var(--rtb-accent) 10%,transparent)}.task-action ha-icon{--mdc-icon-size:17px}.notes-task{color:var(--rtb-accent)}.column-task .task-buttons{float:right;margin-top:-18px}
      .details{min-width:0;flex:1}.name{font-weight:650;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.task-meta{display:flex;gap:4px;flex-wrap:wrap;padding-top:4px}.task-meta span{display:inline-flex;align-items:center;gap:2px;padding:2px 5px;border-radius:999px;background:var(--secondary-background-color);color:var(--secondary-text-color);font-size:.67rem;line-height:1.2}.task-meta ha-icon{--mdc-icon-size:11px}.progress-bar{display:grid;grid-template-columns:minmax(70px,1fr) auto;align-items:center;gap:7px;margin-top:6px;max-width:310px}.progress-track{position:relative;height:7px;border-radius:999px;background:color-mix(in srgb,var(--divider-color) 55%,transparent);overflow:hidden}.progress-fill{position:absolute;inset:0 auto 0 0;width:var(--progress-width);border-radius:inherit;background:var(--progress-color)}.progress-marker{position:absolute;z-index:1;top:0;bottom:0;width:1px;background:color-mix(in srgb,var(--primary-text-color) 50%,transparent)}.progress-marker.m75{left:37.5%}.progress-marker.m100{left:50%}.progress-marker.m150{left:75%}.progress-value{font-size:.67rem;font-weight:700;color:var(--secondary-text-color);white-space:nowrap}.progress-value b{color:var(--rtb-overdue-color)}.area,.due{font-size:.78rem;color:var(--secondary-text-color)}.due{text-align:right;white-space:nowrap;padding:3px 6px;border-radius:999px;background:var(--secondary-background-color)}.due.status-overdue{color:var(--rtb-overdue-color);font-weight:700}.due.status-soon{color:var(--rtb-due-soon-color);font-weight:600}.due.status-ok{color:var(--rtb-not-due-color)}.due.last-done{color:var(--rtb-accent);font-weight:600}
      .task:not(.column-task){display:grid;grid-template-columns:auto minmax(0,1fr);align-items:center}.task:not(.column-task)>.task-icon{grid-column:1;align-self:center}.task:not(.column-task)>.details{grid-column:2;display:grid;grid-template-columns:minmax(0,1fr) max-content;align-items:center;min-width:0;width:100%;max-width:none;gap:0}.task:not(.column-task) .name{grid-column:1/-1;display:block;min-width:0;width:100%;max-width:none}.list-middle{display:contents}.list-middle .task-meta{grid-column:1;min-width:0;padding-top:0;align-items:center;align-self:center}.list-actions{grid-column:2;display:flex;align-items:center;justify-content:flex-end;align-self:center;text-align:right}.task:not(.column-task) .progress-bar{grid-column:1/-1;width:100%;max-width:none;min-width:0}
      ...
      .temporal-layout{display:flex;min-height:0;flex:1}.temporal-main{display:flex;flex-direction:column;min-width:0;flex:1}.overdue-column{width:170px;min-width:145px;max-width:24%;overflow:auto;background:color-mix(in srgb,var(--rtb-overdue-color) 6%,var(--card-background-color));flex:none}.overdue-left{border-top:1px solid var(--divider-color)}.overdue-left .overdue-column{border-right:1px solid var(--divider-color)}.overdue-right .overdue-column{border-left:1px solid var(--divider-color)}.overdue-head{position:sticky;top:0;z-index:3;display:flex;align-items:center;gap:5px;height:36px;padding:0 7px;box-sizing:border-box;background:var(--secondary-background-color);color:var(--rtb-overdue-color);font-size:.78rem;font-weight:600}.overdue-head ha-icon{--mdc-icon-size:17px}.overdue-head strong{margin-left:auto}.overdue-list{padding:5px}.overdue-list .column-task{padding:7px 5px;border:0;border-radius:7px;margin-bottom:5px;background:var(--card-background-color)}.overdue-list .column-task .name{font-size:.76rem;white-space:normal}.overdue-list .column-task .area{padding-top:2px}.overdue-age{padding-top:3px;color:var(--rtb-overdue-color);font-size:.68rem;font-weight:650;line-height:1.2}@media(max-width:600px){.overdue-column{width:135px;min-width:120px;max-width:38%}.overdue-head span{display:none}}
...
class RemeysTaskboardCardEditor extends HTMLElement {
  ...
  _render() {
    ...
    this.shadowRoot.innerHTML = `<style>
      ...
      ha-input{width:100%}.select{display:grid;gap:5px;font-size:.85rem}.select select{width:100%;padding:10px;border:1px solid var(--divider-color);border-radius:6px;background:var(--card-background-color);color:var(--primary-text-color)}.status-colors{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.status-color{display:grid;gap:5px;font-size:.78rem}.status-color input{box-sizing:border-box;width:100%;height:40px;padding:3px;border:1px solid var(--divider-color);border-radius:6px;background:var(--card-background-color);cursor:pointer}.calendar-config{display:grid;gap:10px;padding:10px;border:1px solid var(--divider-color);border-radius:10px}.calendar-source{display:grid;grid-template-columns:1.4fr 1fr 1fr 1.2fr 70px auto;gap:7px;align-items:end}.calendar-source label{display:grid;gap:3px;font-size:.72rem}.calendar-source input{box-sizing:border-box;width:100%;min-width:0;padding:8px;border:1px solid var(--divider-color);border-radius:6px;background:var(--card-background-color);color:var(--primary-text-color)}.calendar-source input[type="color"]{height:36px;padding:3px}.calendar-icon-picker{width:100%}.small-btn{border:1px solid var(--divider-color);border-radius:7px;background:var(--secondary-background-color);color:var(--primary-text-color);padding:8px;cursor:pointer}@media(max-width:900px){.calendar-source{grid-template-columns:1fr 1fr}.calendar-source .entity{grid-column:1/-1}}@media(max-width:500px){.row,.status-colors{grid-template-columns:1fr}}
      ...
      <div><div class="label">Statusfarben</div><div class="hint">Farben für überfällige, bald fällige und noch nicht fällige Aufgaben.</div><div class="status-colors"><label class="status-color">Überfällig<input type="color" data-status-color="overdue_color" value="${safeStatusColor(this._config.overdue_color, "#db4437")}"></label><label class="status-color">Bald fällig<input type="color" data-status-color="due_soon_color" value="${safeStatusColor(this._config.due_soon_color, "#f0a12b")}"></label><label class="status-color">Nicht fällig<input type="color" data-status-color="not_due_color" value="${safeStatusColor(this._config.not_due_color, "#43a047")}"></label></div></div>
    ...
    this.shadowRoot.querySelectorAll("input[data-status-color]").forEach((input) => {
      input.addEventListener("change", () => this._update({ [input.dataset.statusColor]: input.value }));
    });
